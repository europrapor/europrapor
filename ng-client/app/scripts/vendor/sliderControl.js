...
(function() {
  ...
  function SliderControl(el, args, options) {
    ...
    // apply thumb CSS
    for (var b in this.options.thumbCss) {
      this.thumb.style[b] = this.options.thumbCss[b];
    }
    ...
    if (this.options.label) makeLabels(this);
  ...
  SliderControl.prototype = {
    ...
    // set 0-based index
    setIndex: function(i) {
      if (typeof i != 'undefined' && this.values && i >= 0 && i < this.values.length) {
    ...
    onTouchEnd: function(e) {
      ...
      var that = this;
      if (this.valueIndex != this.startIndex || (this.values.length == 1 && this.percent > 99)) {
  ...
  function range(start, end, step) {
    ...
    if (l === 0) return [];
    if (l == 1) return arguments.call(0, start, 1);
    if (l == 2) return arguments.call(start, end, 1);
    var temp = [];
    if (step === 0) return [];
  ...
  function makeLabels(self) {
    ...
    self.labelsDiv.innerHTML = label;
  }

  // expose!!
  window.SliderControl = SliderControl;
  SliderControl.version = '0.3';
  SliderControl.range = range;
